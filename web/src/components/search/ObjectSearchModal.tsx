  ...
  return (
    <div
      className={`rounded-lg border transition-all ${
        isExpanded ? 'border-sky-500/50 bg-night-800/50' : 'border-night-700 bg-night-800/30'
      }`}
    >
      {/* Header - clickable */}
      <button
      ...
      >
        {/* Status indicator */}
        <div className={`p-2 rounded-lg ${statusInfo.bgColor} ${statusInfo.color}`}>
          {statusInfo.icon}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h3 className="font-medium text-white truncate">{result.displayName}</h3>
        ...
        <div className="px-3 pb-3 pt-0 border-t border-night-700 mt-0">
          <div className="pt-3 space-y-3">
                  ...
                  {result.visibility.moonSeparation !== null && (
                    <div>
                      ...
                      >
                        ...
                        {result.visibility.moonWarning && ' (interference)'}
                      </p>
                    </div>
                  )}
                </div>
            ...
            {!result.visibleTonight && result.nextVisibleDate && !result.neverVisible && (
              <div className="bg-sky-500/10 rounded-lg p-3 border border-sky-500/20">
                <div className="flex items-center gap-2 text-sky-400 font-medium mb-2">
                  ...
                  <span>Next Visible: {formatDate(result.nextVisibleDate)}</span>
                </div>
                {result.visibility && (
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div>
            ...
            <div className="text-xs text-gray-400 flex items-center gap-4">
              ...
              {result.isMovingObject && (
                <span className="text-purple-400">(position changes)</span>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="bg-night-900 rounded-xl shadow-xl w-full max-w-lg max-h-[80vh] flex flex-col border border-night-700">
        ...
        <div className="p-3 border-t border-night-700 flex-shrink-0">
          <div className="flex items-center gap-2 text-xs text-gray-500">
            ...
            <span>
              Visibility calculated for {location.name || `${location.latitude.toFixed(2)}°, ${location.longitude.toFixed(2)}°`}
            </span>
          </div>
        </div>
      </div>
