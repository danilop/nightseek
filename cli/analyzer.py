...
from events import MeteorShower
    ...
    def _analyze_single_night(
        ...
        for planet_name, planet_obj in self.planets.items():
            ...
            if visibility.is_visible and visibility.max_altitude_time:
                try:
                    ...
                    logger.debug(
                        "Could not compute diameter for %s: %s", planet_name, e
                    )
        ...
        for comet in self.comets:
            try:
                ...
                logger.debug(
                    "Could not compute visibility for comet %s: %s",
                    comet.designation,
                    e,
                )
                continue
        ...
        for dp in self.dwarf_planets:
            try:
                ...
                logger.debug(
                    "Could not compute visibility for dwarf planet %s: %s", dp.name, e
                )
                continue
        ...
        for ast in self.asteroids:
            try:
                ...
                logger.debug(
                    "Could not compute visibility for asteroid %s: %s", ast.name, e
                )
                continue
        ...
        conjunctions = detect_conjunctions(
            self.calculator, planet_visibility, self.planets, night_info
        )

        # Detect meteor showers
        meteor_showers = detect_meteor_showers(
            self.calculator, night_info, moon_visibility
        )
